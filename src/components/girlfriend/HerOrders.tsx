...
import { Clock, Heart, CheckCircle2, XCircle, Gift, Sparkles } from "lucide-react";
...
export function HerOrders({
  ...
  const getStatusBadge = (status: RedemptionOrder["status"]) => {
    switch (status) {
      case "pending":
        return (
          <span className="px-2 py-0.5 rounded-full bg-amber-50 text-amber-800 text-[11px] font-bold border border-amber-200 flex items-center gap-1.5">
            <Clock className="w-3 h-3 text-amber-600" />
            <span>Waiting for Mahesh</span>
          </span>
        );
      case "approved":
      case "fulfilling":
        return (
          <span className="px-2 py-0.5 rounded-full bg-romantic-50 text-romantic-800 text-[11px] font-bold border border-romantic-200 flex items-center gap-1.5">
            <Heart className="w-3 h-3 text-romantic-600 fill-romantic-500" />
            <span>Approved & Locked in</span>
          </span>
        );
      case "completed":
        return (
          <span className="px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-800 text-[11px] font-bold border border-emerald-200 flex items-center gap-1.5">
      ...
      case "rejected":
        return (
          <span className="px-2 py-0.5 rounded-full bg-stone-100 text-stone-700 text-[11px] font-bold border border-stone-200 flex items-center gap-1.5">
    ...
    <div className="flex-1 overflow-y-auto hide-scrollbar pb-24 px-5 pt-3">
      ...
      {filteredOrders.length === 0 ? (
        <div className="text-center py-12 px-4 bg-white/60 border border-warm-border rounded-3xl mt-4">
          <div className="w-12 h-12 rounded-2xl bg-warm-muted flex items-center justify-center mx-auto mb-2 text-romantic-500">
          ...
          <button
            ...
            className="mt-3 px-3.5 py-1.5 bg-romantic-500 text-white rounded-xl text-xs font-semibold shadow-sm flex items-center gap-1.5 mx-auto"
          >
