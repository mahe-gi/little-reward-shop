...
export async function getHomeDashboardData() {
  try {
    ...
    // 1. Points earned today by current user
    const earnedTodayAggregate = await prisma.walletTransaction.aggregate({
      _sum: { amount: true },
      where: {
        userId: user.id,
        type: "EARN_TASK",
    ...
    // 2. Partner tasks completed this week
    let partnerTasksCompletedThisWeek = 0;
    if (partner) {
      partnerTasksCompletedThisWeek = await prisma.task.count({
        where: {
    ...
    // 3. User tasks completed this week
    const userTasksCompletedThisWeek = await prisma.task.count({
      where: {
    ...
    // 4. Real Recent Activities from PostgreSQL
